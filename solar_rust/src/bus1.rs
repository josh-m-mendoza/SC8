    ...
    loop{
        ...
        let clean_id = if eff_flag == 1 {match socketcan::ExtendedId::new(can_id & 0x1FFFFFFF){
                ...
                None => {error!(eff_flag = true,"Invalid extended CAN ID");
                         continue;}}} 
        else {match socketcan::StandardId::new((can_id & 0x7FF) as u16){
        ...
        let matched_frame = messages_kelly::Messages::from_can_message(can_frame.id(), can_frame.data());
        ...
        match matched_frame{
           ...
           if let Err(e) = redis::cmd("TS.MADD")
            .arg("Kelly:motor_current").arg(&time_seconds).arg(frame.motor_current())
            .arg("Kelly:battery_voltage").arg(&time_seconds).arg(frame.battery_voltage())
            .arg("Kelly:id_error").arg(&time_seconds).arg(frame.id_error())
            .arg("Kelly:over_voltage").arg(&time_seconds).arg(frame.over_voltage())
            .arg("Kelly:low_voltage").arg(&time_seconds).arg(frame.low_voltage())
            .arg("Kelly:stall").arg(&time_seconds).arg(frame.stall())
            .arg("Kelly:internal_volts_fault").arg(&time_seconds).arg(frame.internal_volts_fault())
            .arg("Kelly:over_temperature").arg(&time_seconds).arg(frame.over_temperature())
            .arg("Kelly:throttle_error").arg(&time_seconds).arg(frame.throttle_error())
            .arg("Kelly:hall_throttle_open").arg(&time_seconds).arg(frame.hall_throttle_open())
            .arg("Kelly:angle_sensor_error").arg(&time_seconds).arg(frame.angle_sensor_error())
            .arg("Kelly:motor_over_temperature").arg(&time_seconds).arg(frame.motor_over_temperature())
            .arg("Kelly:hall_galvanometer_error").arg(&time_seconds).arg(frame.hall_galvanometer_error())
            ...
            Ok(messages_kelly::Messages::Message2(frame))=>{
                ...
                if let Err(e) = redis::cmd("TS.MADD")
                .arg("Kelly:throttle_signal").arg(&time_seconds).arg(frame.throttle_signal())
                .arg("Kelly:controller_temperature").arg(&time_seconds).arg(frame.controller_temperature())
                .arg("Kelly:motor_temperature").arg(&time_seconds).arg(frame.motor_temperature())
                .arg("Kelly:command_status").arg(&time_seconds).arg(command_status)
                .arg("Kelly:feedback_status").arg(&time_seconds).arg(feedback_status)
                .arg("Kelly:hall_a").arg(&time_seconds).arg(frame.hall_a())
                .arg("Kelly:hall_b").arg(&time_seconds).arg(frame.hall_b())
                .arg("Kelly:hall_c").arg(&time_seconds).arg(frame.hall_c())
                .arg("Kelly:brake_switch").arg(&time_seconds).arg(frame.brake_switch())
                .arg("Kelly:backward_switch").arg(&time_seconds).arg(frame.backward_switch())
                .arg("Kelly:forward_switch").arg(&time_seconds).arg(frame.forward_switch())
                .arg("Kelly:foot_switch").arg(&time_seconds).arg(frame.foot_switch())
                .arg("Kelly:boost_switch").arg(&time_seconds).arg(frame.boost_switch())                
                .query_async::<_,Vec<u64>>(&mut con)
                .await{
            ...
            _=>{error!(failed_frame = ?matched_frame ,"Cant decode this message. Expected a KellyKLS motorcontroller Frame");}
