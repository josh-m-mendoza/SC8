...
impl Messages {
    ...
    pub fn from_can_message(id: Id, payload: &[u8]) -> Result<Self, CanError> {
        let res = match id {
            ...
            Reset::MESSAGE_ID => Messages::Reset(Reset::try_from(payload)?),
...
impl Reset {
    pub const MESSAGE_ID: embedded_can::Id =
        Id::Standard(unsafe { StandardId::new_unchecked(0x503) });

    pub const UNUSED_MIN: i32 = 0_i32;
    pub const UNUSED_MAX: i32 = 0_i32;
    pub const UNUSED_MIN: i32 = 0_i32;
    pub const UNUSED_MAX: i32 = 0_i32;

    /// Construct new Reset from values
    pub fn new(unused: i32, unused: i32) -> Result<Self, CanError> {
        let mut res = Self { raw: [0u8; 8] };
        res.set_unused(unused)?;
        res.set_unused(unused)?;
        Ok(res)
    }
    ...
    /// Set value of Unused
    #[inline(always)]
    pub fn set_unused(&mut self, value: i32) -> Result<(), CanError> {
        let factor = 1;
        let value = value.checked_sub(0).ok_or(CanError::ParameterOutOfRange {
...
impl core::convert::TryFrom<&[u8]> for Reset {
    type Error = CanError;
...
impl embedded_can::Frame for Reset {
    fn new(id: impl Into<Id>, data: &[u8]) -> Option<Self> {
        if id.into() != Self::MESSAGE_ID {
            None
        } else {
...
impl core::fmt::Debug for Reset {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        if f.alternate() {
            ...
            f.debug_tuple("Reset").field(&self.raw).finish()
        }
    }
}
...
#[cfg(feature = "arb")]
impl<'a> Arbitrary<'a> for Reset {
    fn arbitrary(u: &mut Unstructured<'a>) -> Result<Self, arbitrary::Error> {
        let unused = u.int_in_range(0..=0)?;
        let unused = u.int_in_range(0..=0)?;
        Reset::new(unused, unused).map_err(|_| arbitrary::Error::IncorrectFormat)
    }
}
